...
class MovieRepositoryRemote @Inject constructor(){
    ...
    fun getPopularMovies(page: Int, language:String, onResponse: (OnResult<List<MovieResponse>>) -> Unit) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val apiResponse: Response<MoviesResponse> = retroService.getPopularMovies(page, language, apiKey)
    ...
    fun getMoviesByKeyword(key: String, page: Int, language:String,  onResponse: (OnResult<List<MovieResponse>>) -> Unit) {
        val apiKey = BuildConfig.API_KEY
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val retroService = ApiConnection.RETRO_SERVICE_NO_AUTH
                val apiResponse: Response<MoviesResponse> = retroService.getMoviesByKeyword(key, page, language, apiKey)
                if (apiResponse.isSuccessful) {
                    apiResponse.body()?.let { response ->
                        withContext(Dispatchers.Main) {
                            onResponse(OnResult.Success(response.movies))
